...
//voiture
        ...
        Voiture V;
        ui->le_matricule->setValidator (new QIntValidator(100, 9999999, this));
        ui->tab_voiture->setModel(V.afficher());

        ui->tab_voiture_2->setModel(V.tricroissant());
        ui->tab_voiture_5->setModel(V.afficher());

          ui->tab_voiture_3->setModel(V.trideccroissant());
          //Arduino
...
void MainWindow::update_label()
{
    ...
    if(data=="1")
    {
        ...
        ui->label_73->setText("Acces authorised"); // si les données reçues de arduino via la liaison série sont égales à 1
    // alors afficher ON
    ui->label_77->setText("Voiture detectée"); // si les données reçues de arduino via la liaison série sont égales à 1

}
    else if (data=="0")

        ui->label_73->setText("Aceess denied");   // si les données reçues de arduino via la liaison série sont égales à 0
     //alors afficher ON
}

void MainWindow::on_pushButtonn_clicked()   // implémentation du slot bouton on
{
     A.write_to_arduino("1"); //envoyer 1 à arduino
}

void MainWindow::on_pushButton_22_clicked()  // implémentation du slot bouton off
{

     A.write_to_arduino("0");  //envoyer 0 à arduino
     ui->label_73->setText("Aceess denied");
}

void MainWindow::on_pushButton_33_clicked()  // implémentation du slot bouton +
{
    A.write_to_arduino("2");   //envoyer 2 à arduino
}

void MainWindow::on_pushButton_44_clicked() // implémentation du slot bouton -
{
...
{Employe e;
    // background //
              ...
              amande->setStackingGap(1);
               //couleurs
              amande->setName("Repartition des  employes");
              ...
              // ajout des données  (statistiques du employe)//

              QVector<double> PlaceData;
              QSqlQuery q1("select age from EMP");
              while (q1.next()) {
                            int  nbr_fautee = q1.value(0).toInt();
                            PlaceData<< nbr_fautee;
                              }
...
void MainWindow::on_pushButton_13_clicked()
{
   ...
   findClient.exec();
   findCar.prepare("select TYPE from VOITURE Where TYPE='"+voiture+"'");
   findCar.exec();
...
void MainWindow::on_pb_ajouter6_clicked()
{
    ...
    Voiture V(matricule,type,date,prix);
    bool test=V.ajouter();
    QMessageBox msgBox;
...
{
     Voiture V;
    int mat = ui->le_matricule_supp->text().toInt();
...
void MainWindow::on_pb_rechercher6_clicked()
{
    class Voiture V ;
        ui->tab_voiture->setModel(V.afficher());
...
void MainWindow::on_pb_rechercher_clicked()
{
    int mat = ui->le_matricule->text().toInt();
      ...
      QString strStream;
                             ...
                             "<head>\n"
                                             ...
                                             //     "<align='right'> " << datefich << "</align>"
                                              "<center> <H1>Liste des Voiture</H1></br></br><table border=1 cellspacing=0 cellpadding=2>\n";
